...
void nonce_encryption (const unsigned char* N,
  ...
  printf("cipher call in nonce_encryption\n");
  block_cipher(s,k,T,CNT,D,t,n);
  // PRINT BELOW THE BLOCK CIPHER CALL
...
unsigned long long msg_encryption (const unsigned char** M, unsigned char** c,
  ...
  *M = *M + len8;
  lfsr_gf56(CNT);
  printf("cipher call in msg encryption \n");
...
unsigned long long ad_encryption (const unsigned char** A, unsigned char* s,
  ...
  *A = *A + len8;
  ...
  if (adlen != 0) {
    ...
    pad(*A, T, t, len8);
    *A = *A + len8;
    printf("cipher call in ad encryption \n");
    block_cipher(s,k,T,CNT,D,t,n);
    lfsr_gf56(CNT);
  }

...
#define DEBUG 0
#define PRINT_SKINNY_CALL
void display_vector_skinny (const unsigned char* x, int lenx) {
  int i;
...
void skinny_128_384_enc (unsigned char* input, const unsigned char* userkey) {
  ...
  #ifdef PRINT_SKINNY_CALL

  printf("plaintext = ");
  display_vector_skinny(input, 16);
  printf("tweakey = ");
  display_vector_skinny(userkey, 48);
  printf("lfsr = "); 
  display_vector_skinny(userkey,7);
  printf("domain = ");
  display_vector_skinny(userkey+7, 1);
  printf("tweak= = "); 
  display_vector_skinny(userkey+16, 16);
  printf("key = ");
  display_vector_skinny(userkey+32, 16);
  #endif
  enc(input,userkey,5); 
  printf("cipher = ");
  display_vector_skinny(input, 16);
